---
title: 'OpenTelemetry'
description: 'Learn how to configure OpenTelemetry observability with VibeKit'
---

VibeKit provides built-in support for [OpenTelemetry](https://opentelemetry.io/), the industry-standard observability framework. This integration allows you to collect traces, metrics, and logs from your VibeKit operations for monitoring and debugging purposes.

## What is OpenTelemetry?

[OpenTelemetry](https://opentelemetry.io/) is a collection of APIs, libraries, agents, and instrumentation to provide observability for cloud-native software. It enables you to:

- **Trace requests** across distributed systems
- **Collect metrics** about application performance
- **Generate logs** with structured data
- **Export data** to various observability backends

## Configuration

### Using withTelemetry

```typescript
import { VibeKit } from '@vibe-kit/sdk';

const vibeKit = new VibeKit()
  .withAgent({
    type: 'claude',
    provider: 'anthropic',
    apiKey: process.env.ANTHROPIC_API_KEY!,
    model: 'claude-sonnet-4-20250514',
  })
  .withSandbox(e2bProvider)
  .withTelemetry({
    type: 'remote',
    endpoint: 'https://test-otel-endpoint.com/v1/traces',
    serviceName: 'test-vibekit',
    serviceVersion: '1.0.0',
    headers: {
      Authorization: 'Bearer test-token',
    },
    timeout: 5000,
    samplingRatio: 1.0,
    resourceAttributes: {
      environment: 'test',
    },
  });
```

### Configuration Object Method

To enable OpenTelemetry in VibeKit, add the `telemetry` configuration to your `VibeConfig`:

```typescript
import { VibeKit, VibeConfig } from '@vibe-kit/sdk';

const config: VibeConfig = {
  agent: {
    type: 'codex',
    model: {
      apiKey: 'sk-proj-****',
    },
  },
  environment: {
    e2b: {
      apiKey: 'e2b_****',
    },
  },
  github: {
    token: 'ghp_****',
    repository: 'superagent-ai/vibekit',
  },
  telemetry: {
    type: 'remote',
    endpoint: 'https://test-otel-endpoint.com/v1/traces',
    serviceName: 'test-vibekit',
    serviceVersion: '1.0.0',
    headers: {
      Authorization: 'Bearer test-token',
    },
    timeout: 5000,
    samplingRatio: 1.0,
    resourceAttributes: {
      environment: 'test',
    },
  },
};

const vibeKit = new VibeKit(config);
```

## Configuration Options

| Option | Type | Description | Default |
|--------|------|-------------|---------|
| `type` | `string` | Must be `'remote'` for OpenTelemetry export | - |
| `endpoint` | `string` | OTLP endpoint URL for trace export (required) | - |
| `serviceName` | `string` | Name of your service for identification | `'vibekit'` |
| `serviceVersion` | `string` | Version of your service | `'1.0.0'` |
| `headers` | `object` | Custom headers for the OTLP exporter | `{}` |
| `timeout` | `number` | Timeout for trace export in milliseconds | `30000` |
| `samplingRatio` | `number` | Sampling ratio (0.0 to 1.0) | `1.0` |
| `resourceAttributes` | `object` | Additional resource attributes | `{}` |

### Backward Compatibility

The new `type` field is fully backward compatible:

```typescript
// ✅ New recommended approach
.withTelemetry({ type: 'remote', endpoint: '...' })

// ✅ Still works (legacy approach)  
.withTelemetry({ isEnabled: true, endpoint: '...' })

// ❌ This combination won't work as expected
.withTelemetry({ isEnabled: false, type: 'remote' })  // type takes precedence
```

## Supported Backends

VibeKit's OpenTelemetry integration works with any OTLP-compatible observability backend, including:

- [Jaeger](https://www.jaegertracing.io/)
- [Zipkin](https://zipkin.io/)
- [DataDog](https://www.datadoghq.com/)
- [New Relic](https://newrelic.com/)
- [Honeycomb](https://www.honeycomb.io/)
- [Grafana Tempo](https://grafana.com/oss/tempo/)
- [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/)

## What Gets Traced

When telemetry is enabled, VibeKit automatically traces:

- Code generation requests and responses
- Agent interactions and decision making
- Environment setup and teardown
- GitHub operations
- Error conditions and exceptions

Each trace includes relevant metadata such as:
- Request/response payloads (sanitized)
- Execution duration
- Success/failure status
- Agent model information
- Environment details

## Visualizing with Dashboard

While OpenTelemetry data is typically viewed in external systems like Jaeger or Datadog, you can also use the VibeKit dashboard for local visualization:

```bash
# Start dashboard with OTLP proxy
vibekit dashboard --telemetry-port 3000

# Configure telemetry to use both local and remote
const vibekit = new VibeKit()
  .withTelemetry({
    type: 'remote',
    endpoint: 'https://otlp.example.com/v1/traces',
    // Also enable local API for dashboard
    api: {
      enabled: true,
      port: 3000,
      otlpProxy: true
    }
  });
```

This hybrid approach allows you to:
- View real-time data locally during development
- Export production data to your observability platform
- Debug issues with the familiar VibeKit dashboard interface

## Best Practices

1. **Use semantic span names**: Make traces easy to understand
2. **Add relevant attributes**: Include context that helps debugging
3. **Sample in production**: Use sampling to control data volume
4. **Secure your endpoints**: Always use HTTPS and authentication
5. **Monitor export errors**: Set up alerts for failed exports

## Troubleshooting

### Traces Not Appearing

1. Check endpoint connectivity:
   ```bash
   curl -X POST https://your-endpoint/v1/traces
   ```

2. Verify authentication headers are correct

3. Check VibeKit logs for export errors

4. Ensure sampling ratio isn't too low

### Performance Impact

If telemetry impacts performance:
- Reduce sampling ratio
- Increase batch size
- Use async export
- Consider local buffering

## Next Steps

- [Dashboard](/dashboard) - Visual monitoring and management interface
- [Local Telemetry](/local-telemetry) - Local-only telemetry option
- [CLI Commands](/cli/commands/telemetry) - Telemetry CLI reference
- Explore your chosen backend's documentation for advanced queries

## Related Resources

- [OpenTelemetry Documentation](https://opentelemetry.io/docs/)
- [OTLP Specification](https://opentelemetry.io/docs/specs/otlp/)
- [Semantic Conventions](https://opentelemetry.io/docs/specs/semconv/)
- [VibeKit Telemetry API](/api-reference/telemetry)
