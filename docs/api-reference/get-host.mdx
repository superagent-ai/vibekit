...
```typescript
public getHost(port: number): Promise<string>
```
...
| Type | Description |
|------|-------------|
| `Promise<string>` | The host URL that can be used to access the specified port in the sandbox |
...
const hostUrl = await vibekit.getHost(3000);
...
const apiUrl = await vibekit.getHost(8080);
...
const dbHost = await vibekit.getHost(27017);
...
- **Unsupported port:** When a provider cannot expose the requested port
...
| **Fly.io Sprites** | ⚠️ Partial | Returns the Sprite's HTTPS URL for its default port 8080 |
...
- **Multi-Sandbox Support:** This method is available for E2B, Daytona, Cloudflare, Northflank, and Fly.io Sprites sandbox environments
