...
# 2. Create monitored environment
vibekit local create --name monitored-env --agent claude

# 3. Generate code (telemetry auto-enabled)
vibekit local generate -e monitored-env -p "Build user service"
...
```bash
# Start complete dashboard environment (recommended)
vibekit dashboard
...
# Export data
telemetry export --format csv --output data.csv
