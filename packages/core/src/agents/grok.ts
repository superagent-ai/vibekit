import type { Agent, AgentEvent, AgentResult, BaseSandbox, GrokConfig, FinalResult } from "../types.js";
import { createAgentResult } from "./agent-result.js";

/**
 * Ensure Grok CLI is installed in the sandbox
 */
async function ensureGrokInstalled(sandbox: BaseSandbox): Promise<void> {
  const result = await sandbox.process.startAndWait("which grok || command -v grok");
  if (result.exitCode !== 0) {
    console.log("Installing Grok CLI...");
    const installResult = await sandbox.process.startAndWait(
      "npm install -g grok-cli || echo 'Grok CLI not yet available'"
    );
    if (installResult.exitCode !== 0) {
      throw new Error(`Failed to install Grok CLI: ${installResult.stderr}`);
    }
  }
}
 ...
 */
function buildCommand(config: GrokConfig, _mode: "run" | "ask"): string {
  const { model, flags = [] } = config;

  const args: string[] = ["grok"];
 ...
 */
export function createGrokAgent(sandbox: BaseSandbox, config: GrokConfig): Agent {
  const { apiKey } = config;

  const startExecution = async (prompt: string, mode: "run" | "ask"): Promise<AgentResultInternal> => {
    await ensureGrokInstalled(sandbox);
    ...
    const handle = await sandbox.process.start(
      `XAI_API_KEY="${apiKey}" ${cmd} "$(cat "${promptFile}")"`,
      {
        env: {
          XAI_API_KEY: apiKey,
        },
