import type { Agent, AgentEvent, AgentResult, BaseSandbox, OpencodeConfig, FinalResult } from "../types.js";
import { createAgentResult } from "./agent-result.js";

/**
 * Ensure Opencode CLI is installed in the sandbox
 */
async function ensureOpencodeInstalled(sandbox: BaseSandbox): Promise<void> {
  const result = await sandbox.process.startAndWait("which opencode || command -v opencode");
  if (result.exitCode !== 0) {
    console.log("Installing Opencode CLI...");
    const installResult = await sandbox.process.startAndWait(
      "go install github.com/opencode-ai/opencode@latest || npm install -g opencode"
    );
    if (installResult.exitCode !== 0) {
      throw new Error(`Failed to install Opencode CLI: ${installResult.stderr}`);
    }
  }
}
 ...
 */
function buildCommand(config: OpencodeConfig, _mode: "run" | "ask"): string {
  const { provider, model, flags = [] } = config;
 ...
 */
export function createOpencodeAgent(sandbox: BaseSandbox, config: OpencodeConfig): Agent {
  ...
  const startExecution = async (prompt: string, mode: "run" | "ask"): Promise<AgentResultInternal> => {
    await ensureOpencodeInstalled(sandbox);
    ...
    const handle = await sandbox.process.start(
      `${envVarName}="${apiKey}" ${cmd} "$(cat "${promptFile}")"`,
      {
        env: {
          [envVarName]: apiKey,
        },
