import type { Agent, AgentEvent, AgentResult, BaseSandbox, CodexConfig, FinalResult } from "../types.js";
import { createAgentResult } from "./agent-result.js";

/**
 * Ensure Codex CLI is installed in the sandbox
 */
async function ensureCodexInstalled(sandbox: BaseSandbox): Promise<void> {
  const result = await sandbox.process.startAndWait("which codex || command -v codex");
  if (result.exitCode !== 0) {
    ...
    if (installResult.exitCode !== 0) {
      throw new Error(`Failed to install Codex CLI: ${installResult.stderr}`);
    }
  }
}
 ...
 */
function buildCommand(config: CodexConfig, mode: "run" | "ask"): string {
  ...
  // For ask mode, set approval to suggest
  if (mode === "ask") {
    args.push("--approval-mode", "suggest");
  } else {
    // For run mode, use full-auto
 ...
 */
export function createCodexAgent(sandbox: BaseSandbox, config: CodexConfig): Agent {
  const { apiKey } = config;
  ...
  const startExecution = async (prompt: string, mode: "run" | "ask"): Promise<AgentResultInternal> => {
    ...
    const handle = await sandbox.process.start(
      `OPENAI_API_KEY="${apiKey}" ${cmd} "$(cat "${promptFile}")"`,
      {
        env: {
